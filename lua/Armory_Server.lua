...
function Armory:ResupplyPlayer(player)
	...
	if fullhealth then
		...
		if HasMixin(player, "ParasiteAble") and player:GetIsParasited() then player:RemoveParasite() end
		if player:isa("Marine") and player.poisoned then player.poisoned = false end
    end
