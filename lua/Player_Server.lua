...
function Player:OnUpdatePlayer(deltaTime)
    ...
    -- concede sequence
    if self.dirtyConcedeRelevancy then
        self.dirtyConcedeRelevancy = nil
        self:ConfigureRelevancy(self.concedeTargetPoint - self:GetOrigin(), kConcedeRelevancyDistance - kMaxRelevancyDistance)
    end
...
function Player:ClearConcedeSequence()
    self.concedeSequenceActive = false
    
    -- end smoothed relevancy just in case
    if HasMixin(self, "SmoothedRelevancy") and self.EndSmoothedRelevancy then
        self:EndSmoothedRelevancy()
    end
end

function Player:StartConcedeSequence(sequenceSettings)
    self.concedeStartAngle = sequenceSettings.startAngle
    self.concedeAngularVelocity = sequenceSettings.angularVelocity
    self.concedeTargetPoint = sequenceSettings.targetPoint
    self.concedeStartTime = sequenceSettings.startTime
    self.concedeSettingsIndex = sequenceSettings.settingsIndex
    self.dirtyConcedeRelevancy = true
    self.concedeSequenceActive = true
    
    self:PrimaryAttackEnd()
    self:SecondaryAttackEnd()
    self:ClearGameEffects()
    
    self:SetCameraDistance(1) -- so players will see their world model instead of their view model.
...
function Player:StartStalemateSequence(sequenceSettings)
    ...
    self.stalemateTargetPoint = sequenceSettings.targetPoint
    self.stalemateStartTime = sequenceSettings.startTime
    self.stalemateSettingsIndex = sequenceSettings.settingsIndex
    self.dirtystalemateRelevancy = true
    self.stalemateSequenceActive = true
