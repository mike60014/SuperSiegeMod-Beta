...
local function CreateStalemateButton(self, teamType)
    ...
    local StalemateText = GetGUIManager():CreateTextItem()
    StalemateText:SetTextAlignmentX(GUIItem.Align_Center)
    StalemateText:SetTextAlignmentY(GUIItem.Align_Center)
    StalemateText:SetFontName(kFontName)
    StalemateText:SetScale(scaleVector)
    GUIMakeFontScale(StalemateText)
    StalemateText:SetAnchor(GUIItem.Middle, GUIItem.Center)
    StalemateText:SetText(Locale.ResolveString("Vote Stalemate"))
    
    self.background:AddChild(background)
    background:AddChild(StalemateText)
    
    return { Background = background, StalemateText = StalemateText }
